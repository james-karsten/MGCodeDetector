...
//  [T] uint32_t GCodeParser::codebits;  // found bits
//  [T] uint8_t GCodeParser::param[26];  // parameter offsets from command_ptr
#else
...
void GCodeParser::parse(char *p) {
  ...
  while (const char param = uppercase(*p++)) {  // Get the next parameter. A NUL ends the loop
    ...
    if (PARAM_OK(param)) {
      ...
      // [T] unneccesary debug if (TERN0(DEBUG_GCODE_PARSER, debug)) SERIAL_EOL();
...
void GCodeParser::unknown_command_warning() {
  ...
  std::cout << STR_UNKNOWN_COMMAND << " command ptr: " << command_ptr << std::endl;
}
